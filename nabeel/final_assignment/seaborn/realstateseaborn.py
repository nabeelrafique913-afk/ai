...
# Customize the theme
sns.set_theme(style='darkgrid', rc={'axes.facecolor': 'yellow', 'grid.color': 'blue'})
...
df = pd.read_csv('nabeel/kagglerealstate.csv',delimiter=",",  date_format={'date_added': '%m-%d-%Y'} , index_col='brokered_by')
...
#kind='hist'  
g=sns.displot(data=dffilter, x="bed" , y="price" , hue="city" ,kind='hist'  )
g.figure.suptitle("sns.displot(data=dffilter, x=bed , y=price , hue=city,  kind='hist'  )"  )
...
#kind='kde'
g=sns.displot(data=dffilter, x="price" , y="brokered_by" , kind='kde'  )
...
The relationship between x and y can be shown for different subsets of the data using the hue, size, and style parameters. These parameters control what visual semantics are used to identify the different subsets. It is possible to show up to three dimensions independently by using all three semantic types, but this style of plot can be hard to interpret and is often ineffective. Using redundant semantics (i.e. both hue and style for the same variable) can be helpful for making graphics more accessible."""
g=sns.displot(data=dffilter100, x="city" , y="price" ,hue="city" )
...
g=sns.catplot(data=dffilter, x="city", y="price" , hue="city")
