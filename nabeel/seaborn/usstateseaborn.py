...
# Customize the theme
sns.set_theme(style='darkgrid', rc={'axes.facecolor': 'green', 'grid.color': 'white'})
...
df = pd.read_csv('nabeel/usstate.csv',delimiter=",", parse_dates=[3],  date_format={'date_added': '%m-%d-%Y'} , index_col='Company')
...
#kind='hist'  
g=sns.barplot(data=dffilter, x="Company" , y="Valuation ($B)" , hue="City" )
g.figure.suptitle("sns.displot(data=dffilter, x=Company , y=valuation , hue=city,  kind='hist'  )"  )
...
#kind='kde'
g=sns.displot(data=dffilter, x="Company" , y="Date Joined" ,hue='Company')
...
g=sns.kdeplot(data=dffilter, x="Date Joined")
...
g = sns.histplot(data=dffilter, x='Company', y='Country', multiple="stack")
...
g = sns.scatterplot(x='Company', y='Country', data=dffilter)
...
The relationship between x and y can be shown for different subsets of the data using the hue, size, and style parameters. These parameters control what visual semantics are used to identify the different subsets. It is possible to show up to three dimensions independently by using all three semantic types, but this style of plot can be hard to interpret and is often ineffective. Using redundant semantics (i.e. both hue and style for the same variable) can be helpful for making graphics more accessible."""
g=sns.lineplot(data=dffilter, x="Company" , y="Country"  )
...
g=sns.barplot(data=dffilter, x="Company", y="Country", legend=False)
...
g=sns.catplot(data=dffilter, x="Company", y="Country")
...
glue = dffilter.pivot(columns="Company", values="Country")
